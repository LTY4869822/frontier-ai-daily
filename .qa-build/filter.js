...
exports.matchSignal = exports.filterHotspots = exports.CATEGORIES = void 0;
/**
 ...
 */
function filterHotspots(items, query, cats) {
    const q = query.trim().toLowerCase();
    const catSet = new Set(cats);
    return items.filter((h) => {
        if (catSet.size > 0 && !catSet.has(h.category))
            return false;
        if (q) {
            const haystack = [
                h.title,
...
exports.filterHotspots = filterHotspots;
/**
 ...
 */
function matchSignal(items, query, fields) {
    const q = query.trim().toLowerCase();
    if (!q)
        return items;
